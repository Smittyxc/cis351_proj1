...
public class SampleUnsigned16BitAdderTest {
  ...
  // Thorough tests
  //
  // Note: The technique below (cramming a bunch of different tests into one test
  // method), is actually rather gross.
  // But, to the best of my knowledge, Java doesn't have a time-efficient, simple
  // technique for putting each desired
  // test in its own method.
  //
  // Below, testIntegers is an array of integers that will serve as inputs to your
  // adder.
  // The `verify` method takes two integers, and the carryIn as input, calculates
  // the expect output, then runs the test.
  // the single test method `testAll` calls `verify` for each combination of
  // integer inputs and carryIn

  // The complete list of integers to be tests.
  // (IMPORTANT: !!! You need to add to this list !!!)
  private static final long testIntegers[] = { 0, 1, 2, 13, 127, 128, 129, 643, 655, 12345, 0xAAAA, 65534, 65535 };
